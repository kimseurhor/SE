...
from face_detection_recognition.face_recognition import preprocessing, FaceFeaturesExtractor, FaceRecogniser
...
def load_data(features_extractor, DATASET_PATH):
...
def train(args, embeddings, labels):
    softmax = LogisticRegression(solver='lbfgs', multi_class='multinomial', C=8, max_iter=15000)
    if args.grid_search:
        clf = GridSearchCV(
            estimator=softmax,
...
def main(MODEL_DIR_PATH, DATASET_FILENAME, DATASET_PATH):
    args = parse_args()

    features_extractor = FaceFeaturesExtractor()
    embeddings, labels, class_to_idx = load_data(features_extractor, DATASET_PATH)
    ...
    model_path = os.path.join(MODEL_DIR_PATH, DATASET_FILENAME)
    # print(model_path)
    joblib.dump(FaceRecogniser(features_extractor, clf, idx_to_class), model_path)


if __name__ == '__main__':
    main("../model", "b10test.pkl", "D:/Internship/Test")
